...
状态压缩DP题，确实不是很好想。
题目要求重新排列项链的珠宝，找到一种排列，使得$$\sum_{i=1}^{m} cnt_i \times val_i\$$达到最大。
 设 `f(a, b, c, st)` 为当前还剩 `a` 个 'a'，`b` 个 'b'，`c` 个 'c'，上一组组合的后两个珠宝状态为 `st = x * 3 + y` 时，能够达到的最大喜爱值。
 设 `x = st / 3`, `y = st % 3`（上一组合的最后两个字符），我们可以选择 `a`, `b`, `c` 作为下一个字符：
     \[
     f(a, b, c, st) = \max(
       val[x][y][0] + f(a-1, b, c, y*3+0),
       val[x][y][1] + f(a, b-1, c, y*3+1),
       val[x][y][2] + f(a, b, c-1, y*3+2)
     )
     \]
